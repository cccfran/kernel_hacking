...
static pid_t pid2 = -1;
static int ratio = -1;
static int freeze_on = 0;
static int old_guy_charger = 0;

asmlinkage int sys_glue(pid_t pid_1, pid_t pid_2, int r) {
    if (find_task_by_pid(pid_1) && find_task_by_pid(pid_2)) {
    pid1 = pid_1;
    pid2 = pid_2;
    ratio = r;
    freeze_on = 1;
    ...
    sys_separate();
    printk(KERN_INFO "Cannot find pids\n");
  }
  return -1;
}

asmlinkage int sys_glue_list(pid_t pids[2]) {
  if (pid1 != -1 && pid2 != -1) { /* if programs are registered */
    pids[0] = pid1;
    pids[1] = pid2;
    return 0;
  }
...
asmlinkage int sys_separate(void) {
  if (pid1 == -1 && pid2 == -1)
    return -1;
  freeze_on = 0;
  pid1 = -1;
...
void slow_old_guy(void) {
  if (pid1 == -1 && pid2 == -1)
    goto out;

  struct task_struct *p1 = find_task_by_pid(pid1);
  struct task_struct *p2 = find_task_by_pid(pid2);
  struct task_struct *old_guy = NULL, *new_guy = NULL;

  if(p1 && p2){ /* if both processes aren't terminated */
    /* decide who is older */
    if ((p1->times.tms_utime + p1->times.tms_stime) > (p2->times.tms_utime + p2->times.tms_stime)) {
      old_guy = p1;
      new_guy = p2;
    }
    else {
    ...
    /* new guy has caught up */
    if ((old_guy->times.tms_utime + old_guy->times.tms_stime) < ((new_guy->times.tms_utime + new_guy->times.tms_stime) + 1)) {
      sys_separate();
      old_guy_charger = 0;
      goto out;
    }
    ...
    if (ratio == 0) /* FOG */
      ...
      if (++old_guy_charger == ratio)  /* Time to charge the old_guy */
        old_guy_charger = 0; /* reset the charger counter */
      else
        old_guy->counter = 0;
    }
  } else {
...
repeat_schedule:
	...
	if (unlikely(!c)) {
        ...
        /* fran's code: slow down the old guy */
        if (freeze_on)
          slow_old_guy();
